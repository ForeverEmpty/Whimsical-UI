...
describe("createMessage", () => {
  test("调用方法应该创建对应的 Message 组件", async () => {
    ...
    await rAF();
    expect(document.querySelector(".w-message")).toBeTruthy();
    handler.close();
    await rAF();
    expect(document.querySelector(".w-message")).toBeFalsy();
  });
  ...
  test("多次调用应该创建多个实例", async () => {
    ...
    await rAF();
    expect(document.querySelectorAll(".w-message").length).toBe(2);
    closeAll();
    await rAF();
  ...
  test("创建多个实例应该设置正确的 offset", async () => {
    ...
    await rAF();
    const elements = document.querySelectorAll(".w-message");

...
describe("Input.vue", () => {
  test("render", () => {
    ...
    // 针对 slots 的测试
    expect(wrapper.find(".w-input__prepend").exists()).toBeTruthy();
    expect(wrapper.get(".w-input__prepend").text()).toBe("prepend");

    expect(wrapper.find(".w-input__prefix").exists()).toBeTruthy();
    expect(wrapper.get(".w-input__prefix").text()).toBe("prefix");
  ...
  test("clearable", async () => {
    ...
    // 不应该出现 Icon 区域
    expect(wrapper.find(".w-input__clear").exists()).toBeFalsy();
    ...
    // 出现 Icon 区域
    expect(wrapper.find(".w-input__clear").exists()).toBeTruthy();

    // 点击 Icon 区域，触发 clear 事件
    await wrapper.get(".w-input__clear").trigger("click");
  ...
  test("toggle password", async () => {
    ...
    // 不应该出现 Icon 区域
    expect(wrapper.find(".w-input__password").exists()).toBeFalsy();
    const input = wrapper.get("input");

    expect(input.element.type).toBe("password");
    await input.setValue("123");

    const eyeIcon = wrapper.find(".w-input__password");
    ...
    // 缓存 Icon
    expect(wrapper.find(".w-input__password").attributes("icon")).toBe("eye");
  });
});
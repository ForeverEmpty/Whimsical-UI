...
import { useId, useProp } from '@whimsical-ui/hooks'
import type { FormItemContext } from './types'

interface UseFormItemInputIdProps extends Record<string, any> {
    id?: string
}
...
export function useFormItemInputId(props: UseFormItemInputIdProps, formItemContext?: FormItemContext) {
    const inputId = ref<string>('')
    let unwatch: WatchStopHandle | void

    onMounted(() => {
        unwatch = watch(toRef(() => props.id), (id) => {

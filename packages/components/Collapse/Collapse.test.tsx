...
describe("Collapse.vue", () => {
  beforeAll(() => {
    ...
    headers = wrapper.findAll(".w-collapse-item__header");
    contents = wrapper.findAll(".w-collapse-item__wapper");
  ...
  test("手风琴模式 错误处理", () => {
    ...
    expect(warn.mock.calls).toMatchInlineSnapshot(
      `
        [
          [
            [WUIError: [WCollapse]: accordion mode should only have one active item],
          ],
        ]
      `
    );
...
describe("Collapse/transitionEvents.ts", () => {
  ...
  test("beforeEnter", () => {
    transitionEvents.beforeEnter && transitionEvents.beforeEnter(wrapper.element);
    expect(wrapper.element.style.height).toBe("0px");
    expect(wrapper.element.style.overflow).toBe("hidden");
  });
  test("enter", () => {
    transitionEvents.enter && transitionEvents.enter(wrapper.element);
    expect(wrapper.element.style.height).toBe(
      `${wrapper.element.scrollHeight}px`
    );
  });
  test("afterEnter", () => {
    transitionEvents.afterEnter && transitionEvents.afterEnter(wrapper.element);
    expect(wrapper.element.style.height).toBe("");
    expect(wrapper.element.style.overflow).toBe("");
  });
  test("beforeLeave", () => {
    transitionEvents.beforeLeave && transitionEvents.beforeLeave(wrapper.element);
    expect(wrapper.element.style.height).toBe(
      `${wrapper.element.scrollHeight}px`
    );
    expect(wrapper.element.style.overflow).toBe("hidden");
  ...
  test("leave", () => {
    transitionEvents.leave && transitionEvents.leave(wrapper.element);
    expect(wrapper.element.style.height).toBe("0px");
  });
  test("afterLeave", () => {
    transitionEvents.afterLeave && transitionEvents.afterLeave(wrapper.element);
    expect(wrapper.element.style.height).toBe("");
    expect(wrapper.element.style.overflow).toBe("");
  });
});